...
This module provides functionality for anomaly detection using various machine learning models, 
including DBSCAN, One-Class SVM, and Isolation Forest. It also integrates with MLflow for experiment tracking.
Classes:
    AnomalyDetection: A class for performing anomaly detection using different models.
Methods:
    __init__(self, data=None):
        Initializes the AnomalyDetection class with optional data.
    preprocess_data(self):
        Preprocesses the data by filling missing values with the mean.
    read_and_split_rocket_features(self, num_kernels=1000, features_file=None):
        Reads and splits the Rocket features into training and validation sets.
    dbscan_anomaly_detection(self, eps=0.5, min_samples=5):
        Performs anomaly detection using the DBSCAN algorithm.
    som_anomaly_detection(self, x_dim=10, y_dim=10, sigma=1.0, learning_rate=0.5, num_iterations=1000):
        Performs anomaly detection using the Self-Organizing Map (SOM) algorithm.
    anomaly_detection(self, model, model_name, experiment_name, log_model=False, registered_model_name=None):
        Performs anomaly detection using the specified model and logs the results to MLflow.
'''
...
mlflow.set_tracking_uri("file:///shared/rc/gamts/Codebase/mlruns")
...
class AnomalyDetection:
    ...
    def anomaly_detection(self, model, model_name, experiment_name, log_model=False, registered_model_name=None):
        ...
        with mlflow.start_run():
            ...
            # log all the metrics from the classification report
            for key, value in train_class_report.items():
                    if isinstance(value, dict):
                        for sub_key, sub_value in value.items():
                            mlflow.log_metric(f"train_{key}_{sub_key}", float(sub_value))
                    else:
                        mlflow.log_metric(f"train_{key}", float(value))

            for key, value in val_class_report.items():
                    if isinstance(value, dict):
                        for sub_key, sub_value in value.items():
                            mlflow.log_metric(f"val_{key}_{sub_key}", float(sub_value))
                    else:
...
# Create an instance of the class containing the anomaly_detection method
    ...
    model_name = "Isolation Forest"  # options    "DBSCAN", "SOM", "OCSVM", "Isolation Forest"
    ...
    anomaly_detector.anomaly_detection(dbscan, model_name, experiment_name, log_model=False, registered_model_name=registered_model_name)
