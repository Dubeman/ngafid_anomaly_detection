...
def calculate_threshold(scores: np.ndarray, n_sigma: float = 3) -> float:
    """Calculate threshold based on training scores."""
    mean_score = np.mean(scores)
    std_score = np.std(scores)
    return mean_score - n_sigma * std_score
...
def cross_validate_anomaly_detector(
    ...
    avg_metrics = {
        metric: (np.mean([m[metric] for m in all_metrics if metric != 'best_params']),
                np.std([m[metric] for m in all_metrics if metric != 'best_params']))
